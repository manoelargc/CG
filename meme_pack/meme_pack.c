...
/*===============================================================
CG-23.2
AVALIAÇÃO 1 (27/09/23)

>Manoela Resende 2210100235

Faça um gerador automático de memes, incluindo uma
marca d'água com a sigla do seu nome, em maiúsculas.
Faça o teste gerando um pack com 10 imagens e 10 frases=100
...
void lerArquivo(char *nomeArquivo, char *array[], int *numLinhas)
{
    ...
    while (fgets(linha, sizeof(linha), arquivo))
    {
        // Remova o caractere de nova linha no final, se houver
        if (linha[strlen(linha) - 1] == '\n')
        {
            linha[strlen(linha) - 1] = '\0';
        }

        // Aloque memória para a nova string
        array[i] = malloc(strlen(linha) + 1);
        if (array[i] == NULL)
        {
            printf("Não foi possível alocar memória para a linha\n");
            return;
        }

        // Copie a linha para a nova string
        strcpy(array[i], linha);
        i++;
    }
...
int main()
{
    ...
    int num_imagens = 0;
    char *imagens[10];
    lerArquivo("imagens.txt", imagens, &num_imagens);

    int num_frases = 0;
    char *frases[100];
    lerArquivo("frases.txt", frases, &num_frases);
